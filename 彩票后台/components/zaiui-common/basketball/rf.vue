<template>
		...
		<view v-if="checkIf()"  class="game_time"  >
			<text :style="{'width':box_width+'rpx','height':box_height+'rpx'}" @click="checked('2-1',pl.split(',')[0])" :class="['item',{'checked':this.check_item['2-1']['checked'] == true}]">
				<view style="display: flex;flex-direction: column;align-items: center;">
					<text>让分客胜</text>
					<text style="margin-top: 4rpx;">{{pl.split(",")[0]}}</text>
				</view>
	...
	<view style="position: absolute;left: 2rpx;top: -50rpx;color: white;font-size: 23rpx;">
		    ...
		    <text :style="{'width':box_width+'rpx','height':box_height+'rpx'}" @click="checked('2-2',pl.split(',')[1])" :class="['item',{'checked':this.check_item['2-2']['checked'] == true}]">
				<view style="display: flex;flex-direction: column;align-items: center;">
					<text v-if="this.check_item['2-2']['checked'] == true">让分主胜 <text style="color:white">({{this.goal}})</text> </text>
					<text v-if="this.check_item['2-2']['checked'] != true">让分主胜 <text :style="{'color':   goal<0?'green':'red'}">({{this.goal}})</text> </text>
					<text style="margin-top: 4rpx;">{{pl.split(",")[1]}}</text>
				</view>
			</text>
		</view>
