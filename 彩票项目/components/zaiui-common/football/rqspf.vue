<template>
	<view>
		<view v-if="checkIf()"  class="game_time"  >
			<text :style="{'width':box_width+'rpx','height':box_height+'rpx'}" @click="checked('2-1',pl.split(',')[0])" :class="['item',{'checked':this.check_item['2-1']['checked'] == true}]">胜{{pl.split(",")[0]}}
	...
	<view style="position: absolute;left: 2rpx;top: -50rpx;color: white;font-size: 23rpx;">
			...
			<text :style="{'width':box_width+'rpx','height':box_height+'rpx'}" @click="checked('2-2',pl.split(',')[1])" :class="['item',{'checked':this.check_item['2-2']['checked'] == true}]">平{{pl.split(",")[1]}}</text>
		    <text :style="{'width':box_width+'rpx','height':box_height+'rpx'}" @click="checked('2-3',pl.split(',')[2])" :class="['item',{'checked':this.check_item['2-3']['checked'] == true}]">负{{pl.split(",")[2]}}</text>
		</view>
...
<script>
	...
	export default{
		...
		methods:{
			checkSingle(){
			return	this.p_single.split(",")[1] == 1;
			},
			checkIf(){
